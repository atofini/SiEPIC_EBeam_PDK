...
class strip_To_Slot(pya.PCellDeclarationHelper):
    ...
    def produce_impl(self):
        ...
        shapes(LayerPinRecN).insert(pya.Path([pya.Point(r+w-0.05/dbu, w/2),
                                              pya.Point(r+w+0.05/dbu , w/2)],w))
        shapes(LayerPinRecN).insert(pya.Text("opt2", pya.Trans(pya.Trans.R0, r+w, w/2))).text_size = 0.5 / dbu
        
        # Create the device recognition layer -- make it 1 * wg_width away from the waveguides.
        dev = Box(-taper, -w / 2 - w, r  + w , y)
        shapes(LayerDevRecN).insert(dev)

        print("Done drawing the layout for - strip_to_slot: %.3f-%g" % (self.rails, self.slot))
